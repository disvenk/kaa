...
var data = {
    ...
    sortType: 1,*/
    pageNum: 1,
    designerProductCategoryList: [],
    designerProductLabelList: []
};
//标签
$.ajax({
    type: 'POST',
...
function post() {
    $.ajax({
        type: 'POST',
        url: urlle + 'designerProduct/designerAddProductManageList',
        dataType: 'json',
        contentType: 'application/json; charset=utf-8',
        headers: {
            'Accept': 'application/json; charset=utf-8',
...
function load(res) {
    ...
    $('#list li .goods-item-img').click(function () {
        ...
        location.href = '../designerProduct/designerGoodsDetailHtml?id='+goodsId+''
    });
}
/*
...
function choose() {
    ...
    data.designerProductCategoryList = [];
    data.designerProductLabelList = [];
    for(var i = 0; i < cate.length; i++){
        data.designerProductCategoryList[i] = {
            categoryId: ''
        };
        data.designerProductCategoryList[i].categoryId = cate.eq(i).attr('class');
    }
    var label = $('#yourChoose div');
    for(var i = 0; i < label.length; i++){
        data.designerProductLabelList[i] = {
            labelId: ''
        };
        data.designerProductLabelList[i].labelId = label.eq(i).attr('class');
    }
    post();
}
