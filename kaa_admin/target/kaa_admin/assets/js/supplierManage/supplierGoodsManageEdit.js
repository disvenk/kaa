...
function detail() {
    ...
    $.ajax({
        type: 'POST',
        url: urlle + 'suplier/suplierGoodsEditDetail',
        dataType: 'json',
        contentType: 'application/json; charset=utf-8',
        headers: {
            'Accept': 'application/json; charset=utf-8',
        ...
        success: function (res) {
            if(res.stateCode == 100){
                ...
                for(var i = 0; i < $('#Suplierscope').find('option').length; i++){
                    if($('#Suplierscope').find('option').eq(i).attr('id') == res.data.Suplierscope){
                        $('#Suplierscope').find('option').eq(i).attr('selected', 'selected')
                    }
                }
                ...
                $('#sort').val(''+res.data[0].sort+'');
                $('#remark').val(''+res.data[0].suplierRemark+'');

                //表格
                for(var i = 0; i < res.data[0].supplierList.length; i++) {
                    var tr1 = '';
                    tr1 += '<tr><td class="table_color">' + res.data[0].supplierList[i].color + '</td>' +
                        '<td class="table_size">' + res.data[0].supplierList[i].size + '</td>' +
                        '<td class="table_offline"><input type="number" value="' + res.data[0].supplierList[i].offlinePrice + '"></td>' +
                        '<td class="table_stock"><input type="number" value="' + res.data[0].supplierList[i].stock + '"></td>' +
                        '<td class="table_remark"><input style="width: 90px" type="text" value="' + res.data[0].supplierList[i].categoryRemark + '"></td>' +
...
$('#confirm').click(function () {
    ...
    if(name==''){
        ...
        $.ajax({
            type: 'POST',
            url: urlle + 'suplier/updateSuplierGoods',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            headers: {
                'Accept': 'application/json; charset=utf-8',
