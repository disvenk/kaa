...
function platform() {
    $.ajax({
        type: 'POST',
        url: '../supOrder/producedStatusCountOffline',
        dataType: 'json',
        contentType: 'application/json; charset=utf-8',
        headers: {
            'Accept': 'application/json; charset=utf-8',
...
function initData(type) {
    ...
    $.ajax({
        type: 'POST',
        url: '../supOrder/supOrderListOffline',
        dataType: 'json',
        contentType: 'application/json; charset=utf-8',
        headers: {
            'Accept': 'application/json; charset=utf-8',
...
function load(res,type) {
    ...
    $('#sample-table-2 thead').append(' <tr> <th>工单号</th> <th>内部编号</th> <th>供应商产品编号</th><th>图片</th><th>客户</th>'+
        '<th>分类</th> <th>颜色</th> <th>尺寸</th> <th>件数</th> <th>价格</th><th>下单日期</th>'+
        '<th>交货日期</th><th>快递信息</th> <th>备注</th><th>供应商</th><th>状态</th> <th>操作</th></tr>');
    //绑定产生表格数据
    $('#tbody').html('');
    for (var i = 0; i < res.data.length; i++) {
        var tr = '';
        tr += '<tr id="'+res.data[i].supOrderId+'">  <td>'+res.data[i].supOrderNo+'</td> <td>'+res.data[i].insideOrderNo+'</td> <td>'+res.data[i].pno+'</td><td><img style="width: 70px;height: 70px" src="'+res.data[i].href+'" alt=""></td>' +
            ' <td>'+res.data[i].customer+'</td><td>'+res.data[i].categoryName+'</td> <td>'+res.data[i].color+'</td> <td>'+res.data[i].size+'</td> '+
            '<td>'+res.data[i].qty+'</td> <td>'+res.data[i].outputPrice+'</td> <td>'+res.data[i].supOrderDate+'</td> <td>'+res.data[i].deliveryDate+'</td> '+
            ...
            '<td><div onclick="remarkLog('+res.data[i].supOrderId+')" style="color: rgb(101,161,199);cursor: pointer;max-width: 140px;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;">'+res.data[i].remarks+'</div></td><td>'+res.data[i].supplierName+'</td>' +
...
$("#btnOutput").click(function () {
    var url = "../supOrder/exportExcelOffline?supOrderNo=" + data.supOrderNo +
        "&pno="+data.pno +
...
$("#btnDownload").click(function () {
    ...
    var url = "../supOrder/exportWordListOffline?ids=" + ids.substring(0, ids.length - 1);
    window.open(url);
    // window.location.href = (url);
