 ...
 * 注意：此 ShardedJedisSentinelPool 来自 github，项目地址：https://github.com/warmbreeze/sharded-jedis-sentinel-pool
 *
 * 使用时，此Pool必须独立部署，可架设在单独的redis分片代理服务器上。不能随web一起部署，因为web可能会部署多分用于负载均衡，
 * 导致有两处监听 jedis.subscribe()。
 *
 * 直接从github上下载代码存在几个BUG，已修复。但此Pool是否具有线程安全性，还没测试。
 *
 * 已知BUG：
 * 主从切换时，jedis.subscribe() 中的 onMessage() 有时候并没有被调用，导致 SentinelPool 池中的 host 还是之前的，
 * 但服务器确实是切换成功了（连接 sentinel，输入 info 可知）
 * 这里的"有时候"指：当web程序第一次收到主从切换时的订阅消息后，如果不关闭web程序，继续第二次主从切换，第二次的主从切换订阅消息就没有接受到。
 *
 */
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
	...
	public ShardedJedis getResource() {
		...
		jedis.setDataSource(this);  //必须要设置数据源，否则 close() 时，不会将实例放回池中的
		return jedis;
	}
	...
	* */
	...
	protected static class ShardedJedisFactory implements PooledObjectFactory<ShardedJedis> {
		...
		public boolean validateObject(PooledObject<ShardedJedis> pooledShardedJedis) {
			try {
				...
				for (Jedis shard : jedis.getAllShards()) {  //bug：如果使用2个master，kill掉其中一个，getShared() 得到的连接池还是那个kill掉的，没有自动切换
	...
	/*
	* 王华 重要备注：
	* 该线程的run方法通过Jedis Pub/Sub API（实现JedisPubSub接口，并通过jedis.subscribe进行订阅）
	* 向Sentinel实例订阅“+switch-master”频道，当Sentinel进行主从切换时，该线程会得到新Master地址的通知，
	* 通过master name判断哪个分片进行了切换，将新master地址替换原来位置的地址，并调用initPool(List masters)进行
	* Jedis连接池重建；后续所有通过该连接池取得的连接都指向新Master地址，对应用程序透明；
	*
	* 主备切换时，可以客户端连接 sentinel，查看是否切换完成：redis-cli -h 42.96.197.253 -p 6379，
	* 输入：info；如果 "master0:name=mymaster,status=ok,address=115.28.185.29:6379,slaves=1,sentinels=2"
	* "address" 已发生变化，那么 jedis.subscribe() 中的 onMessage() 方法也应该被调用了。
	* 实际测试下来，发现有时候 onMessage() 不能被调用。具体原因还不明，待查！！！
	* 这里的"有时候"指：当web程序第一次收到主从切换时的订阅消息后，如果不关闭web程序，继续第二次主从切换，第二次的主从切换订阅消息就没有接受到。
	* */
